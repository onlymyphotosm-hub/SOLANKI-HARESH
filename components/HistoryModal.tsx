...
interface HistoryModalProps {
  ...
  onEditEntry: (index: number, updatedEntry: HistoryEntry) => void;
}

const HistoryModal: React.FC<HistoryModalProps> = ({ history, onClose, isOpen, theme, beadsPerRound, onEditEntry }) => {
  ...
  return (
    <>
            ...
            <div>
              ...
              {history.length === 0 ? (
                  ...
                  {history.map((entry, index) => (
                      <li 
                        key={index} 
                        className={`group flex justify-between items-center p-3 ${theme.listItemBackground} rounded-lg shadow-sm border ${theme.listItemBorder} cursor-pointer hover:shadow-md transition-all`}
                        onClick={() => setEditingIndex(index)}
                      >
                      <div className="flex items-center space-x-3">
                         <div className={`p-2 rounded-full ${theme.accentLight} text-amber-600 opacity-0 group-hover:opacity-100 transition-opacity`}>
      ...
      <EditHistoryModal 
        ...
        initialData={editingIndex !== null ? history[editingIndex] : null}
